...
import { CategoryGrid } from '@/components/comedy/CategoryGrid';
    ...
    <div className="min-h-screen bg-[#FAF7F2]">
      ...
      <div className="bg-gradient-to-r from-[#E8F4F8] to-white elegant-shadow sticky top-0 z-10 border-b-2 border-[#C9A227]">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center gap-4">
          <Link href="/office/comedy" className="text-[#2E6B8A] hover:text-[#C9A227] transition-colors">
            <ArrowLeftIcon className="w-6 h-6" />
          </Link>
          <div className="flex-1">
            <h1 className="text-2xl font-headline text-[#2D2D2D]">{comedian.name}</h1>
            <p className="text-xs text-[#2E6B8A] font-body">{comedian.tagline}</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <div className="flex items-center justify-center mb-4">
            ...
            <div>
              <h2 className="text-3xl font-headline text-[#2E6B8A]">{comedian.name}</h2>
              <p className="text-[#2D2D2D]">{comedian.tagline}</p>
            </div>
          </div>
          <p className="text-lg text-[#2D2D2D]">What do you want to laugh about?</p>
        </div>
        
        <CategoryGrid comedian={comedian.id} />
      </div>
    </div>
  );
}